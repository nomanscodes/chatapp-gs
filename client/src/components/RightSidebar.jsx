...
const RightSidebar = ({ selectedUser }) => {
    return (
        <div className={`bg-[#8185B2]/20 text-white w-full relative overflow-y-scroll ${selectedUser ? 'max-md:hidden ' : ''}`}>
            ...
            <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
                {imagesDummyData.map((image, index) => (
                    <div key={index} onClick={() => window.open(image)} className='cursor-pointer rounded '>
                        <img src={image} alt={`media-${index}`} className='rounded-md h-full' />
                    </div>
                ))
                }
            </div>
