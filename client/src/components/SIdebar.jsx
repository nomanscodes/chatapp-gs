...
const SIdebar = ({ selectedUser, setSelectedUser }) => {
    ...
    return (
        <div
            className={`bg-[#8185B2]/20 h-full p-5 overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""
                }`}
        >
            <div className="pb-5">
            ...
            <div className="flex flex-col">
                {userDummyData.map((user, index) => (
                    <div
                        key={index}
                        className={`relative flex items-center gap-2 py-2 pl-4 rounded hover:bg-[#151113] cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && "bg-[#3a3158]/80"
                            }`}
